...
import { getFilePathFromUrl, getFileStats, deleteFile } from "./lib/storage";
...
app.get("/api/configuration", async (req, res) => {
	...
	try {
		...
		const usedBytes = await getStorageUsage(domain);
		const customizations = await getCustomizations(domain);
...
app.post("/api/configuration", async (req, res) => {
	...
	try {
		await saveTenantConfig(domain, parseInt(storage_limit_mb, 10));
		res.status(200).json({ success: true });
	} catch (err) {
...
app.post("/api/webhooks/order-created", async (req, res) => {
	...
	try {
		for (const line of lines) {
			...
			if (fileUrl) {
				const resolvedPath = getFilePathFromUrl(fileUrl);
				if (resolvedPath) {
...
app.get("/api/customizations/:id/download", async (req, res) => {
	...
	try {
		const record = await getCustomization(id);
