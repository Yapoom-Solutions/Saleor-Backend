...
def create_site_settings():
    ...
    # Delete existing settings rows first to avoid site_id unique violations
    SiteSettings.objects.all().delete()

    # Get allowed field names from the model class (including relation _id suffixes)
    allowed_fields = set()
    for field in SiteSettings._meta.get_fields():
        allowed_fields.add(field.name)
        if field.is_relation:
            allowed_fields.add(f"{field.name}_id")

    for settings_item in data:
        pk = settings_item["pk"]
        fields = dict(settings_item["fields"])
        # Filter fields to only include those present on the model class
        defaults = {k: v for k, v in fields.items() if k in allowed_fields}
