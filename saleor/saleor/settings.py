...
# django-tenants requires its own PostgreSQL backend so it can switch the
# search_path per request.  All other connection parameters come from
# DATABASE_URL (set by dj_database_url above) and are preserved as-is.
DATABASES[DATABASE_CONNECTION_DEFAULT_NAME]["ENGINE"] = (
    "django_tenants.postgresql_backend"
)

# TenantSyncRouter must be the first router; it delegates non-tenant tables
# to the public schema while letting django-tenants handle tenant routing.
# The existing PrimaryReplicaRouter is kept second so replica reads still work.
DATABASE_ROUTERS = [
...
MIDDLEWARE = [
    # TenantMainMiddleware MUST be first so every subsequent middleware and
    # view runs inside the correct PostgreSQL schema for the current request.
...
# ---------------------------------------------------------------------------
# django-tenants app classification
# ---------------------------------------------------------------------------
# SHARED_APPS  → installed into the *public* schema only.
#                These tables exist once and are visible to every tenant.
# TENANT_APPS  → installed into each *tenant* schema.
#                These tables are isolated per tenant.
#
# Rule of thumb:
#   - Infrastructure / Django internals → SHARED_APPS
#   - Saleor commerce domain logic     → TENANT_APPS
# ---------------------------------------------------------------------------

SHARED_APPS = [
    # django-tenants itself must be first
    "django_tenants",
    # Django internals that live in public schema
    "django.contrib.contenttypes",
    "django.contrib.sites",
    "django.contrib.staticfiles",
    "django.contrib.postgres",
    # Celery beat scheduler tables (shared across all tenants)
    "django_celery_beat",
    # Tenant + Domain tables — must be shared so the routing middleware can
    # look them up in the public schema on every request.
    "saleor.tenants",
    # File/media storage backend (no DB tables, but needed in shared scope)
    "storages",
]

TENANT_APPS = [
    # All Saleor commerce apps.  Each tenant schema gets its own copy of
    # these tables, providing full schema-level data isolation.
    "saleor.permission",
    "saleor.auth",
    "saleor.plugins",
    "saleor.account",
    ...
    "saleor.thumbnail",
    "saleor.schedulers",
    # Third-party apps with tenant-scoped data
    "django_measurement",
    "mptt",
    "django_countries",
    "django_filters",
...
# django-tenants requires INSTALLED_APPS to be the union of both lists,
# with shared apps listed first.
...
# Include wildcard subdomains so tenant hostnames like acme.localhost resolve.
ALLOWED_HOSTS = get_list(
    os.environ.get("ALLOWED_HOSTS", "localhost,127.0.0.1,.localhost")
)
